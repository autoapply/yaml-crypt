...
describe('yaml-crypt', () => {
    ...
    it('should return the encrypted content', () => {
        const yaml = yamlcrypt({ encryptionKey: 'aehae5Ui0Eechaeghau9Yoh9jufiep7H' });
        const str = '{ key1: "Hello, world!", key2: "Hello, world!" }';
        const result = yaml.encrypt(str);
        const expected = fs.readFileSync('./test/test-1a.yaml-crypt').toString('utf8');
        expect(result).to.equal(expected);
    });

    it('should return the encrypted raw content', () => {
        const yaml = yamlcrypt({ encryptionKey: 'aehae5Ui0Eechaeghau9Yoh9jufiep7H' });
        const expected = fs.readFileSync('./test/test-7.yaml-crypt').toString('utf8');
        const str = 'Hello!';
    ...
    it('should re-encrypt transformed content', () => {
        ...
        const transformed = yaml.transform(content1,
            str => str.replace('Hello!', 'Hello, world!'));
    ...
    it('should encrypt new content when transforming', () => {
        ...
        const newContent = 'key1: !<!yaml-crypt/fernet> Hello, world!\nkey2: !<!yaml-crypt/branca> Hello, world!';
    ...
    it('should re-encrypt transformed raw content', () => {
        const yaml = yamlcrypt({ keys: 'aehae5Ui0Eechaeghau9Yoh9jufiep7H' });
        const content = fs.readFileSync('./test/test-7.yaml-crypt');
        const transformed = yaml.transform(content,
            str => str.replace('Hello!', 'Hello, world!'));
        expect(transformed).to.equal('XUvrtHkyXTh1VUW885Ta4V5eQ3hBMFQMC3S3QwEfWzKWVDt3A5TnVUNtVXubi0fsAA8eerahpobwC8');
    });
