  ...
  return {
    encrypt: (str, opts = {}) => {
      if (opts.raw) {
        ...
        return dump(obj, yamlOpts(mergeOpts(opts)));
      }
    },
    ...
    encryptAll: (str, opts = {}) => {
      if (opts.raw) {
        ...
        const objs = yaml.loadAll(str);
        for (const obj of objs) {
          walkStringValues(obj, opts.path, s => new Plaintext(s));
        }
        return dumpAll(objs, yamlOpts(mergeOpts(opts)));
      }
    },
    ...
    decrypt: (str, opts = {}) => {
      if (opts.raw || isToken(str)) {
        ...
        return yaml.load(str, yamlOpts(mergeOpts(opts)));
      }
    },
    ...
    decryptAll: (str, opts = {}) => {
      if (opts.raw || isToken(str)) {
        ...
        return loadAll(decrypted);
      } else {
        return loadAll(str, yamlOpts(mergeOpts(opts)));
      }
    },
...
function doTransform(str, callback, opts) {
  ...
  const result = loadAll(transformed, { schema: schema });
  return dumpAll(result, yamlOpts(opts));
}
