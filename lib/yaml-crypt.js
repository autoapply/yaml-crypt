...
const { algorithms, isToken, generateKey, encrypt, decrypt } = require("./crypto");
const { safeDumpAll, safeLoadAll, walkStringValues, walkValues, tryDecrypt } = require("./utils");
...
function createYamlcrypt(keys, encryptionKey) {
    ...
    return {
        encrypt: (str, opts = {}) => {
            if (opts.raw) {
                return encrypt(opts.algorithm,
                    opts.encryptionKey || encryptionKey, str);
            } else {
        ...
        decrypt: (str, opts = {}) => {
            if (opts.raw || isToken(str)) {
                const s = trimStr(str);
                const decrypted = tryDecrypt(algorithms, opts.keys || keys,
        ...
        transform: (str, callback, opts = {}) => {
            if (opts.raw || isToken(str)) {
                const s = trimStr(str);
                const [key, algorithm, decrypted] = tryDecrypt(algorithms,
                    opts.keys || keys,
...
function createYamlSchema({ algorithm, keys, encryptionKey, objects, base64 }) {
    ...
    for (let i = 0; i < algorithms.length; i++) {
        const isDefault = (algorithms[i] === algorithm
            || algorithm == null && i === 0);
        types.push(createType(Object.assign({}, opts, {
...
function createType({ algorithm, isDefault, keys, encryptionKey, objects, base64 }) {
    ...
    const type = new yaml.Type(name, {
        ...
        construct: data => {
            const decrypted = tryDecrypt([algorithm], keys,
                (algorithm, key) => decrypt(algorithm, key, data));
            const decoded = (base64 ? Buffer.from(decrypted, "base64").toString("utf8") : decrypted);
            return (objects ? new Plaintext(decoded, data, algorithm) : decoded);
        },
        predicate: data => {
            return data.algorithm === algorithm ||
                (data.algorithm == null && isDefault);
        },
        represent: data => {
            let encrypted;
            if (data.ciphertext) {
...
function doTransform(str, callback, opts) {
    const [key, docs] = tryDecrypt(algorithms, opts.keys,
        (algorithm, key) => {
    ...
    for (const doc of docs) {
        walkValues(doc, null, v => v instanceof Plaintext, t => {
...
function newTextTypes() {
    const keys = [
        { "type": "!yaml-crypt", "algorithm": algorithms[0] }
    ];
...
module.exports = { algorithms, generateKey, loadConfig, yamlcrypt, encrypt, decrypt };
